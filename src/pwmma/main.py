...
def calc_spars_of_wgchain(wgchain: 'Chain',
    ...
    cnp = cp if gpu_ready and sm_config.use_gpu else np
    ...
    for idx_f, f in enumerate(tqdm(freqs, disable=not show_progress)):
        ...
        for idx_wgt, wgt in enumerate(wgchain.transitions):
            ...
            s11, s12, s21, s22 = calc_scattering_matrix(cm, zarr1, zarr2,
                                                        cnp=cnp, dtype=dtype,
                                                        conjugate_output=sm_config.conjugate_output)
            if counter == 0:
                SA = (s11, s12, s21, s22)
            else:
    ...
    st11 = np.stack([x.get() for x in st11]) if cnp==cp else np.stack(st11)
    st12 = np.stack([x.get() for x in st12]) if cnp==cp else np.stack(st12)
    st21 = np.stack([x.get() for x in st21]) if cnp==cp else np.stack(st21)
    st22 = np.stack([x.get() for x in st22]) if cnp==cp else np.stack(st22)
